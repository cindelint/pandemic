...
int redC = color(245, 106, 46);
int blueC = color(34, 132, 230);
int blackC = color(94, 94, 94);
int yellowC = color(250, 231, 23);
...
class Card {
    ...
    int xPos, yPos;
    int number;
    Card(String name, String country, int cardC, String type, int xPos, int yPos) {
        this.name = name;
...
class Role {
  ...
  int roleColor;
  int number;
  Role(String name, String info, int roleColor) {
    this.name = name;
    this.info = info;
...
public void drawSphere(int x, float sphereX, float sphereY, float diameter) {
    float cardW = diameter*3.9f;
    float cardX = sphereX+cardW/3.5f;
    ...
    if (drawP[x].cardC == redC) {
        ellipse(sphereX,sphereY,cardW/6.75f,cardW/6.75f);
        ...
        float leftest = sphereX - cardW/14.7f;
        float lefter = sphereX - cardW/21.5f;
    ...
    arc(sphereX,sphereY,cardW/5.0f,cardW/4.9f,190,254);

    //sphere shadow (with guess and check values)
    noStroke();
    fill(0,0,0,55);
    beginShape();
    vertex(sphereX+cardW/10.4f,sphereY-cardW/11.0f);
    bezierVertex(sphereX+cardW/4.4f,sphereY+cardW/10.4f,sphereX-cardW/26.6f,sphereY+cardW/4.8f,sphereX-cardW/8.7f,sphereY+cardW/15.4f);
    bezierVertex(cardX-cardW/5.3f,cardY-cardW/4.8f,cardX-cardW/5.4f,cardY-cardW/2.5f,sphereX+cardW/10.4f,sphereY-cardW/11);
    endShape();
}

//for pure sphere purposes, 1-blue 2-yellow 3-black 4-red
public void drawPlayer(int role, float x, float y, float s) {

    fill(roles[role].roleColor);
    strokeWeight(1);
    stroke(0, 0, 0, 175);
    beginShape();
    vertex(x-s/4.0f,y+s/2.3f);
    bezierVertex(x-s/8,y+s/2,x+s/8,y+s/2,x+s/4,y+s/2.3f);
    bezierVertex(x+s/4.5f,y-s/4.0f,x,y-s/11.5f,x+s/8.0f,y-s/3.9f);
    bezierVertex(x+s/3.9f,y-s/1.8f,x-s/3.9f,y-s/1.8f,x-s/8.0f,y-s/3.9f);
    bezierVertex(x,y-s/11.5f,x-s/4.5f,y-s/4.0f,x-s/4.0f,y+s/2.3f);
    endShape();

    stroke(255,255,255,130);
    strokeWeight(s/30);
    noFill();
    arc(x,y-s/3.3f,s/5.9f,s/3.8f,191,256);
    arc(x,y+s/3.0f,s/3.0f,s/1.1f,191,252);
}

//CARD DESIGN
public void cardContents(int cx, float cX, float cY, float cW) {
    //city name
    pushMatrix();
    fill(255,255,255);
    ...
    if (w < cW/1.4f) {
        ...
        translate(cX,cY);
        scale(0.8f,1);
        text(drawP[cx].name,-cW/2.0f,-cW*7/10+cW/30);
    }
    popMatrix();

    drawSphere(cx, cX-cW/3.25f, cY-cW/3, cW/3.9f);
}
public void drawDrawP(int x, float cardX, float cardY, float cardW) {
    if (drawP[x].type == "city") {

        int cardBg = color(235, 217, 146);
        ...
        fill(252,252,252);
        noStroke();
        rect(cardX,cardY+cardW/28,cardW,cardW/2.2f);

        //white card outline
        strokeWeight(cardW/15);
        stroke(250, 250, 250);
        fill(0, 0, 0, 0);
        rect(cardX,cardY,cardW+cardW/17.5f,cardW*7/5+cardW/17.5f,cardW/7);
...
//CHIP DESIGN
public void drawArrow(float chipD) {
    beginShape();
    vertex(chipD/20.8f,-chipD/11.3f);
    vertex(chipD/8.6f,-chipD/6.1f);
    vertex(chipD/12.3f,-chipD/5.2f);
    vertex(chipD/5.1f,-chipD/5.1f);
    vertex(chipD/5.2f,-chipD/11.1f);
    vertex(chipD/6.1f,-chipD/8.5f);
    vertex(chipD/11.5f,-chipD/26.1f);
    endShape();
}; //outbreak counter: arrows
public void drawInfThing(float chipD) {
    float x = 78.8f;
    float y = 25.3f;
    fill(23, 23, 23);
    beginShape();
    vertex(-chipD/x,-chipD/y);
    bezierVertex(-chipD/7.7f,-chipD/13.4f,-chipD/7.2f,-chipD/5.6f,-chipD/24.4f,-chipD/4.2f);
    bezierVertex(-chipD/6.7f,-chipD/5.8f,-chipD/7.7f,-chipD/29.4f,-chipD/34.7f,-chipD/71.5f);
    vertex(-chipD/x,-chipD/y);
    endShape();
}; //infection counter: strange curved things
public void drawYel(float chipD) {
     noStroke();
     fill(245, 226, 12, 200);
     beginShape();
     vertex(-chipD/100,0);
     vertex(-chipD/14.8f,-chipD/2.01f);
     vertex(chipD/14.8f,-chipD/2.01f);
     vertex(chipD/100,0);
     endShape();
}; //eradicated diseases: the yellow lines
public void infChip(float x, float y, String type) {
    int d = 20;
    pushMatrix();
    translate(x,y);
    ...
    if (type == "outbreak") {
        ...
        for (int i=0; i<100; i++) {
            noStroke();
            fill(255, 0, 115, 50);
            ellipse(0,0,d*2.4f/4-i*2,d*2.4f/4-i*2);
            fill(255,255,255,15);
            ellipse(0,0,d/2.0f-i,d/2.0f-i);
        }
        ...
        //the sun
        fill(255, 122, 20);
        ellipse(0,d/21,d/3.4f,d/3.4f);
        noStroke();
        for (int i=0; i<d/3.8f; i++) {
            fill(255, 255, 31, 15);
            ellipse(0,d/23,i,i);
        }
        ...
        noStroke();
        fill(130, 89, 44);
        float a = 2.08f, b = 7.5f;
        float c = 5.9f, e = -60.8f;
        float f = 2.9f, g = 1.61f;
        beginShape();
        vertex(-d/a,d/b);
    ...
    strokeWeight(1);
    noFill();
    ellipse(0,0,d/1.25f,d/1.25f);

    //highlight
    strokeWeight(d/12);
    noFill();
    stroke(255,255,255,150);
    arc(0,0,d/1.25f-3,d/1.25f-3,116,233);
    stroke(255,255,255,100);
    arc(0,0,d/1.25f+2,d/1.25f+2,381,427);
    stroke(255,255,255,50);
    arc(0,0,d/1.25f+2,d/1.25f+2,318,343);

    popMatrix();
}
public void cureChip(float x, float y, String c) {
    int d = 20;
    pushMatrix();
    translate(x,y);
    ...
    for (int i=0; i<100; i++) {
        noStroke();
        fill(112, 112, 112, 20);
        ellipse(0,0,d/1.5f-i*2,d/1.5f-i*2);
    }
    ...
    fill(245, 245, 245);
    rectMode(CENTER);
    rect(0,-d/4.1f,d/3.9f,d/12.4f,d/100);
    ...
    rect(0,d/35,d/3.3f,d/3.4f,d/20);
    noStroke();
    fill(255, 255, 255, 100);
    rect(-d/9.0f,d/17.8f,d/47,d/6.5f);
